...
/** OCR 결과에 세로셈(MULTVERT/LONGDIV) 블록이 있으면 세로셈으로 본다 */
export function questionLooksLikeVerticalArithmetic(text) {
  const s = String(text ?? '');
  return /\$MULTVERT\s*\{/i.test(s)
    || /\$LONGDIV\s*\{/i.test(s)
    || /\\begin\{array\}/.test(s);
}
...
export function resolveProblemType(userHint, aiType, question) {
  ...
  if (questionLooksLikeVerticalArithmetic(question)) return '세로셈';
  if (hint) return hint;
