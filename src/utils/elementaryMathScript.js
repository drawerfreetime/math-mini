...
export function tryParseLatexForElementaryEditor(latex) {
  ...
  if (newLongDiv) {
    const steps = newLongDiv[4]
      ? newLongDiv[4].split('#').map((x) => normalizeVerticalArithmeticRow(x.trim()))
      : [];
    return {
      templateId: 'longdiv',
      longDiv: {
        divisor:  normalizeVerticalArithmeticRow(newLongDiv[1].trim()),
        dividend: normalizeVerticalArithmeticRow(newLongDiv[2].trim()),
        quotient: normalizeVerticalArithmeticRow(newLongDiv[3].trim()),
        steps,
      },
    };
  }
  ...
  if (mvScr) {
    ...
    if (rowsM) {
      const rowVals = rowsM[1].split('#').map((x) => normalizeVerticalArithmeticRow(x.trim()));
