...
/** 세로 곱셈·나눗셈 격자 — ㉠㉡㉢·□ 빈 칸이 있어도 빈칸채우기보다 우선 */
function shouldPreferVerticalOverBlank(textsIn, horiz, r) {
  if (!hasVerticalOperandStack(textsIn)) return false;
  const hasOp = textsIn.some((t) => hasArithmeticOperator(t.str));
  const numberItems = textsIn.filter((t) => /^\d[\d,.\s]*$/.test(t.str.trim())).length;
  const sumLines = (horiz || []).filter((s) => {
    const len = s.x2 - s.x1;
    const rw = r?.w ?? 100;
    return len >= Math.min(rw * 0.06, 16) && len <= rw * 0.7;
  }).length;
  return hasOp || sumLines >= 1 || numberItems >= 3;
}

/** □·ㄱㄴㄷ 빈칸 채우기 문항 */
function detectBlankFillEarly(textsIn, horiz, r) {
  const joined = textsIn.map((t) => t.str).join(' ');
  ...
  if (shouldPreferVerticalOverBlank(textsIn, horiz, r)) return null;
...
function detectVerticalArithmetic(textsIn, horiz, vert, r) {
  ...
  const hangulBlankStem =
    /[㉠-㉣]/.test(joined) &&
    /알맞은\s*수|구하(?:시오|세요|여|라)?/.test(joined) &&
    hasOp &&
    sumLines.length >= 1;

  if (numberItems.length < 2 && !hangulBlankStem) return null;

  const dataHeaders = textsIn.filter((t) => isDataTableHeaderCell(t.str));
  if (dataHeaders.length >= 3 && !hasOp && !calcIntent) return null;

  if (!hasVerticalOperandStack(textsIn) && !hangulBlankStem) return null;
  ...
  if (!strong && !medium && !hangulBlankStem) return null;

  const gridVert = vert.filter((s) => (s.y2 - s.y1) >= r.h * 0.2);
  const gridHoriz = horiz.filter((s) => (s.x2 - s.x1) >= r.w * 0.25);
  if (!hasOp && !calcIntent && !hangulBlankStem && gridVert.length >= 3 && gridHoriz.length >= 3) {
    return null;
  }

  return {
    type: '세로셈',
    confidence: strong ? 0.9 : hangulBlankStem ? 0.86 : medium ? 0.84 : 0.78,
    debug: {
      mode: hangulBlankStem ? 'vertical-hangul-blank-stem' : 'vertical-arithmetic',
      hasOp,
      calcIntent,
      hangulBlankStem,
      numberItems: numberItems.length,
 ...
 *
 * 0) 세로셈      : [이름] 비교 박스·×÷ 세로 정렬·계산용 가로선 (㉠㉡㉢ 빈 칸 포함, 빈칸·표보다 우선)
...
export function detectStructure(meta, rectN) {
  ...
  // ── 0-c) 빈칸채우기 (□·ㄱㄴㄷ 가로식 — 세로셈·표가 아닐 때만) ──
  const blankEarly = detectBlankFillEarly(textsIn, horiz, r);
