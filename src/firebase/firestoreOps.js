...
export async function saveVariantReview(data, opts = {}) {
  ...
  const peerFields = rest.status === 'peer_review'
    ? {
  ...
  if (
    isUpdate
    && (rest.status === SUBMISSION_STATUS_REGISTERED || rest.status === SUBMISSION_STATUS_APPROVED)
  ) {
    body = {};
    ...
    if (rest.status === SUBMISSION_STATUS_REGISTERED) {
