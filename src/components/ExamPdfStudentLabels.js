...
export default function ExamPdfStudentLabels() {
  ...
  async function runGenerateDownloads() {
    ...
    try {
      ...
      const anyName = mergedStudents.some((s) => {
        const t = s.realName ?? s.displayName ?? '';
        return Boolean(t);
      });
      const anyCjkName = mergedStudents.some((s) =>
        nameNeedsCjkFont(s.realName ?? s.displayName ?? '')
      );
      ...
      for (let i = 0; i < n; i++) {
        ...
        const attendanceText =
          s.studentNumber != null && s.studentNumber !== ''
            ...
            : '';
        const nameText = s.realName ?? s.displayName ?? '';
