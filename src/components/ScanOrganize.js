...
export default function ScanOrganize() {
  ...
  const rosterForOcr = useMemo(() => {
    ...
    const outByNumber = new Map();
    const addCandidate = (studentNumber, candidateNameRaw, aliasesRaw = []) => {
      ...
      const base = outByNumber.get(sn) || { name: candidateName, number: sn, aliases: [] };
      if (!base.name) base.name = candidateName;
      const aliases = Array.isArray(base.aliases) ? base.aliases : [];
      const nextAliases = [...aliases];
      const rawArr = Array.isArray(aliasesRaw) ? aliasesRaw : [aliasesRaw];
      rawArr.forEach((a) => {
        const t = String(a || '').trim();
        if (!t) return;
        if (t === candidateName) return;
        if (!nextAliases.includes(t)) nextAliases.push(t);
      });
      outByNumber.set(sn, { ...base, aliases: nextAliases });
    };

    merged.forEach((s) => {
      const nickname = resolveKimchiNickname(s) || '';
      addCandidate(s.studentNumber, s.realName ?? s.displayName, nickname ? [nickname] : []);
    });
    ...
    const out = Array.from(outByNumber.values()).map((it) => {
      const aliases = Array.isArray(it.aliases) ? it.aliases.filter(Boolean) : [];
      return aliases.length ? it : { name: it.name, number: it.number };
    });
