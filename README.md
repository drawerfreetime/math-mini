...
1. Firestore → **"규칙"** 탭 클릭
2. 기존 내용 전체 삭제
3. `math-app0702-mini/firestore.rules` 파일 내용을 전부 복사하여 붙여넣기
4. **"게시"** 클릭
...
`math-app0702-mini` 폴더에서 `.env.example`을 복사해서 `.env`를 만든 뒤,
메모장으로 열어서 2단계에서 복사한 값으로 교체하세요:
...
1. **명령 프롬프트(cmd)** 열기
2. `math-app0702-mini` 폴더로 이동:
   ```
   cd "C:\Users\PC\OneDrive\바탕 화면\디지털 교육 연구대회\Cursor_AI\math-app0702-mini"
   ```
